...
*/
       if (patient["sex"] === true) {
           $("#patient-sex-disabled").attr("placeholder", "Мужской");
       } else if (patient["sex"] === false) {
           $("#patient-sex-disabled").attr("placeholder", "Женский");
       }
...
*/
       if (patient["room"] === "noroom") {
           $("#patient-room-disabled").attr("placeholder", 'Нет палаты');
       } else {
           $("#patient-room-disabled").attr("placeholder", 'Палата №: ' + patient["room"]);

           var patientDeleteRoomLink = "/app/updateroom/" + hospitalNumber[3] + "/noroom";
           $("#patient-room-disabled").after("<br><a id=\"delete-room-button\" class=\"btn btn-primary btn-lg btn-block\" href=\"" + patientDeleteRoomLink +"\">Выписать из палаты</a>");
       }
